...
module "kms" {
  ...
  keys = {
    general = {
      description   = "General purpose key (AWS-style default)"
      primary_alias = "general"
    }

    s3_state = {
      description         = "S3-only key"
      primary_alias       = "s3-state"
      allowed_via_services = ["s3"]
    }

    restricted = {
      description   = "Restricted to a specific role"
...
## Multi-Region replica example (outside module)
